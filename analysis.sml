...
struct
  ...
  in
  val init_state = (init_all (OurState (0.0, Untouched)) , init_all (EnemyState
  (0.0, 0.0, (SizeDepth 0), Dull)), build_board ())
end
 val xbox_size = 100
 fun sizeOf e =
...
fun them (B {v' = z,...}) = !z
 fun szDiff XBOX _ = XBOX
  ...
  local val decay_rate = 0.95
        ...
        fun update_them(heals, board) = fn(i, m) => let val
          (EnemyState(healscore, modscore, sz, _)) = IntMap.lookup(m, i)
          ...
          val label = if modscore' * 100.0 + healscore' > 200.0 then Asset else (if
          (modscore' < 1.0) andalso (healscore' < 1.0) then Dull else Unknown)
  ...
  let val state' as (us_state', them_state', _) = (decay us_state, decay
    ...
    (Util.upto(255))
  in ((us_state'', them_state'', copy_board board), (IntMap.listKeys(revives),
  IntMap.listKeys(kills)))
end
end
  fun analysis_us (m, _, _) i = let val OurState(_, l) = IntMap.lookup m in l end
fun analysis_them (_, m, _) i = let val EnemyState(_, _, _, l) = IntMap.lookup m in
  l end
end
